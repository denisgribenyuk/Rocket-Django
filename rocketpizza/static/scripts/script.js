$(document).ready(function() {
    ...
    $('.menu__btn-add-to-cart').on('click', function(event) {
        event.preventDefault();
        let product_data = {prod_id: '', mod_id: '', quantity: 1}
        product_data.prod_id = $(this).parents('.menu__item').attr('data-id')
        product_data.mod_id = $(this).parents('.menu__item')
        ...
        $('.cart-counter').text(JSON.parse(localStorage.getItem('rocket-delivery')).length)
